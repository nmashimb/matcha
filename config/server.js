...
con.connect(function(err) {
  ...
  con.query("CREATE DATABASE IF NOT EXISTS matcha", function (err, result){
    ...
    var sql = "CREATE TABLE IF NOT EXISTS chats(id INT(11) AUTO_INCREMENT PRIMARY KEY, connectionid INT(11) , message VARCHAR(255), senderid INT(11), recusername INT(11), readm int(1),  nread int(1))";
    con.query(sql, function(err, result) {
      if (err) throw err;
      console.log("table chats created");
    })

    var sql = "CREATE TABLE IF NOT EXISTS reports (id INT(11) AUTO_INCREMENT PRIMARY KEY, userid INT(11) , reporter INT(11), reason VARCHAR(255))";
    con.query(sql, function(err, result) {
      if (err) throw err;
...
con.connect(function(err) {
  if (err) throw err;
  console.log("Connected 2!");
  con.query("Select * FROM userinfor", function (err, r){
    if (err) throw err;

    r.forEach(e => {
      con.query("UPDATE `userinfor` SET `age`='"+getAge(e["date_of_birth"])+"' WHERE id= '"+e["id"]+"'", function (err, result){
        if (err) throw err;
      })
      if (e["bio"] == undefined)
      {
        con.query("UPDATE `userinfor` SET `bio`=' i am "+e["username"]+"' WHERE id= '"+e["id"]+"'", function (err, result){
          if (err) throw err;
        })
      }
      if (e["preference"] == undefined)
      {
        con.query("UPDATE `userinfor` SET `preference`='bisexual' WHERE id= '"+e["id"]+"'", function (err, result){
          if (err) throw err;
        })
      }
    });
